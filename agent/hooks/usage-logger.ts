...
//
// Eve does not fire parent-agent hooks for subagent turns (see
// node_modules/eve/docs/guides/hooks.md, "Subagent isolation"), so each
// subagent that should have its usage recorded needs its own copy of this
// hook under agent/subagents/<id>/hooks/, built with this factory and given
// that subagent's roleKey.
export function createUsageHook(opts: { roleKey: string }): ReturnType<typeof defineHook> {
  return defineHook({
    events: {
      async "step.completed"(event, ctx) {
        try {
          ...
          await recordUsage({
            ...
            tool: "chat",
            provider:
              modelProvider === "anthropic" ? "anthropic" : modelProvider === "none" ? "none" : "vertex",
            model: modelId,
            inputTokens,
            outputTokens,
            cacheReadTokens,
...
export default createUsageHook({ roleKey: "on-call-engineer" });
